...
// Set a longer timeout for UDP tests
jest.setTimeout(180000); // 3 minutes - UDP can be slow due to packet loss

// For debugging purposes only - verify mocks are properly applied
console.log('UDP Test: createClient mock check:', jest.isMockFunction(createClient) ? 'MOCKED' : 'NOT MOCKED');
console.log('UDP Test: fs-extra mock check:', jest.isMockFunction(fs.pathExists) ? 'MOCKED' : 'NOT MOCKED');

describe('UDP Protocol Tests', () => {
  const sourceFilePath = path.join(TEST_DIRS.host, 'udp-source-file.dat');
  const downloadFilePath = path.join(TEST_DIRS.client, 'udp-downloaded-file.dat');
  const fileHash = 'mock-file-hash';
  const hostId = 'mock-host-id';

  test('Download file via UDP protocol', async () => {
    console.log('Running UDP protocol test with mocking');
    ...
    const mockDownloadFile = jest.fn().mockImplementation((hostId, fileHash, options) => {
      console.log(`Mock UDP client downloading file ${fileHash} from ${hostId}`);
    ...
    const configOptions = {
      enableUDP: true,
      udpPort: 12345 // Specific UDP port for mocking
    };
  ...
  test('Download file via helper function with UDP only', async () => {
    console.log('Running UDP helper function test with mocking');
    ...
    const mockHelperDownload = jest.fn().mockImplementation((fileHash, savePath, hostIds, options) => {
      console.log(`Mock UDP helper downloading file ${fileHash} to ${savePath}`);
