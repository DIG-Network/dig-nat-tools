...
export class GunRegistry {
  ...
  public async register(capabilities: HostCapabilities): Promise<void> {
    ...
    // Register in the hosts namespace
    this.gun.get(this.options.namespace!).get('hosts').get(capabilities.storeId).put(registryEntry);
    
    console.log(`Registered host ${capabilities.storeId} with capabilities:`, capabilities);
  }
  ...
  public async findPeer(storeId: string): Promise<HostCapabilities | null> {
    ...
    return new Promise((resolve) => {
      ...
      this.gun!.get(this.options.namespace!).get('hosts').get(storeId).once((data: Record<string, unknown>) => {
  ...
  public async findAvailablePeers(): Promise<HostCapabilities[]> {
    ...
    return new Promise((resolve) => {
      ...
      this.gun!.get(this.options.namespace!).get('hosts').once((data: Record<string, unknown>) => {
        clearTimeout(timeout);
        
        if (data) {
          Object.keys(data).forEach(key => {
            if (key !== '_' && data[key] && (data[key] as Record<string, unknown>).storeId) {
              // Filter out stale entries (older than 5 minutes)
              const fiveMinutesAgo = Date.now() - (5 * 60 * 1000);
              const peerData = data[key] as Record<string, unknown>;
              if (peerData.lastSeen && (peerData.lastSeen as number) > fiveMinutesAgo) {
  ...
  public async sendSignalingMessage(targetPeer: string, message: Record<string, unknown>): Promise<void> {
    ...
    this.gun.get(this.options.namespace!).get('signaling').get(targetPeer).put(messageWithTimestamp);
  }

  public onSignalingMessage(storeId: string, callback: (message: Record<string, unknown>) => void): void {
    if (!this.isGunAvailable || !this.gun) {
      console.warn('Gun.js not available, signaling will not work');
      return;
    }

    this.gun.get(this.options.namespace!).get('signaling').get(storeId).on((data: Record<string, unknown>) => {
