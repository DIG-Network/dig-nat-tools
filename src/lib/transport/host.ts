...
// Import from local module exports
import {
  CONNECTION_TYPE
...
import type { FileDiscoveredEvent, FileRemovedEvent } from '../../lib/utils/directory-watcher';
import { DirectoryWatcher } from '../../lib/utils/directory-watcher';
import { PeerDiscoveryManager, AnnouncePriority, NODE_TYPE } from '../../lib/discovery/peer/peer-discovery-manager';
import { AuthenticatedFileHost } from '../application/authenticated-file-host';

// Import types from local types file
interface HostOptions {
  hostId?: string;
  hostFileCallback: ((contentId: string, startChunk: number, chunkSize: number, sha256?: string) => Promise<Buffer[] | null>) | AuthenticatedFileHost;
  chunkSize?: number;
  stunServers?: string[];
  enableTCP?: boolean;
  enableUDP?: boolean;
  ...
  watchAnnouncePriority?: string;
  isShardHost?: boolean;
  dhtOptions?: Record<string, any>;
  gunOptions?: Record<string, any>;
  nodeType?: NODE_TYPE;
}

// Port mapping result interface
...
export default class FileHost {
  ...
  private gun: any;
  private hostFileCallback: ((contentId: string, startChunk: number, chunkSize: number, sha256?: string) => Promise<Buffer[] | null>) | AuthenticatedFileHost;
  ...
  constructor(options: HostOptions) {
    this.hostId = options.hostId || uuidv4();
        ...
        if (this.enableNATPMP) {
          try {
            const result = await createPortMapping(
              ...
              'TCP',
              'dig-nat-tools TCP',
              this.portMappingLifetime
            );
            
            // Create a custom result with our expected properties
            const mappingResult = {
              ...result,
              success: true,
              externalAddress: this.externalIPv4
            };
            
            debug(`Created TCP port mapping: internal ${this.tcpPort} -> external ${mappingResult.externalPort}`);
            
            // Store the external IP address if available
            if (mappingResult.externalAddress) {
              this.externalIPv4 = mappingResult.externalAddress;
            ...
            this.connectionOptions.push({
              ...
              address: mappingResult.externalAddress || undefined,
              port: mappingResult.externalPort
            });
            
            // Store the mapping for cleanup
            if (mappingResult.externalPort) {
      ...
      this.udpSocket.bind(this.udpPort, '::', async () => {
        ...
        if (this.enableNATPMP) {
          try {
            ...
            debug(`Created UDP port mapping: internal ${this.udpPort} -> external ${mappingResult.externalPort}`);
            
            // Store the external IP address if available
            if (mappingResult.externalAddress && !this.externalIPv4) {
              this.externalIPv4 = mappingResult.externalAddress;
              debug(`External IPv4 address: ${this.externalIPv4}`);
            }
  ...
  private async _handleChunkRequest(
    ...
    try {
      ...
      if (this.hostFileCallback instanceof AuthenticatedFileHost) {
        this.hostFileCallback.setCurrentRequestPeerId(clientId);
      }
      
      // Get the requested chunk
      const chunks = await this.getFileChunks(contentId, startChunk);
      
      // Clear the current request's peer ID
      if (this.hostFileCallback instanceof AuthenticatedFileHost) {
        this.hostFileCallback.setCurrentRequestPeerId(undefined);
      }
  ...
  /**
   * Get file chunks from the host file callback
   */
  private async getFileChunks(contentId: string, startChunk: number): Promise<Buffer[] | null> {
    if (this.hostFileCallback instanceof AuthenticatedFileHost) {
      return this.hostFileCallback.handleFileRequest(contentId, startChunk, this.chunkSize);
    } else if (typeof this.hostFileCallback === 'function') {
